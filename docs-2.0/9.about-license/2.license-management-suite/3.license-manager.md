...
LM 的配置文件名为`nebula-license-manager.yaml`。

- RPM 包和 DEB 包的默认路径为`/usr/local/yueshu-license-manager/etc/nebula-license-manager.yaml`。
- TAR 包的路径为解压目录下的`yueshu-license-manager/etc/nebula-license-manager.yaml`。
